...
avgTemp = read.csv(file = "Data/EIP_covar.csv", header = TRUE, sep = ",")

read_file("Models/EIP/EIP_rprocess.txt") %>%
...
## ----covar--------------------------------------------------------------
avgTemp = avgTemp[-c(1)] # remove first column

# # Plot
# p = ggplot(avgTemp, aes(x=time, y=avgTemp))
# p = p + geom_line() + xlab("Time (days)")
# print(p)
...
seir_paramnames <- c("S_0","rho","psi","b0","b1","b2","c1","c2","eta","epsilon")
...
if (plot){
    ...
    # Figure EIP_IH
    #--------------------------------------------------------------------------
    cutoff <- data.frame(yintercept=round(theta['eta']), cutoff=factor(round(theta['eta'])))
    label = paste("eta == ", cutoff$yintercept)
    p5 = ggplot(sims, mapping = aes(x=time,y=I_H,group=sim, color=factor(sim))) +
        labs(x = "", y = TeX('Infectious host population ($I_{H}$)')) + 
        theme(
        ...
        annotate("text", x = 2009.5, y = 2075, label = label, parse = TRUE, size = 8)
    print(p5)
    ...
    #     multiplot(pl0,pl1,pl2,pl3,pl4,pl5,pl6,pl7,pl8,pl9,pl10,pl11,
    #               cols = 3,
    #               # layout = matrix(c(1,2,3,4,5,6,7,8,9), nrow=3, byrow=TRUE),
    #               labs=list("", ""),
    #               labpos=list(c(0.5,0.03), c(0.01,0.5))
