...
bakedRDS %<>%
    ...
    ddply(~S_0,subset,rank(-loglik)<=2)
...
# theta is ( 1-S_0, 2-rho, 3-psi, 4-b0, 5-b1, 6-b2, 7-c1, 8-c2, 9-eta, 10-epsilon )

theta.lo = replace(theta, c(2,3,4,5,7,8,9,10), c(0,0,0,0,   0,0, 0,0))
theta.hi = replace(theta, c(2,3,4,5,7,8,9,10), c(1,1,1,1,2000,2,10,1))

profileDesign(
    S_0=seq(from=0,to=350000,length=15),
    lower=theta.lo,upper=theta.hi,nprof=15
) -> pd
# pairs(~b0+S_0+rho+psi+b0+b1+b2+c1+c2+eta+epsilon,data=pd)
