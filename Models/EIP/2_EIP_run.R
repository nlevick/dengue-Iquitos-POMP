...
bakedRDS  %>%
    ...
    ddply(~S_0,subset,rank(-loglik)<=12) %>%
    subset(nfail.max==0,select=seir_paramnames) -> pd2
...
# pairs(~S_0+rho+psi+b0+b1+eta+c1+c2+epsilon,data=pd2)

bake("S0-profile15b.rds",{
    foreach (p=iter(pd2,"row"),
             .combine=rbind,
